...
namespace UnrealSharp
{
    public unsafe class Memory
    {
        ...
        public static readonly delegate* unmanaged[Stdcall]<int, int, int, nint> OpenProcess =
            (delegate* unmanaged[Stdcall]<int, int, int, nint>)NativeLibrary.GetExport(kernel, nameof(OpenProcess));

        public static readonly delegate* unmanaged[Stdcall]<nint, nint, byte[], int, out int, int> ReadProcessMemory2 =
            (delegate* unmanaged[Stdcall]<nint, nint, byte[], int, out int, int>)NativeLibrary.GetExport(kernel,
        ...
        {
            var arrayOfBytes = sig.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)
