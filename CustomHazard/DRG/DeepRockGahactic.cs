...
namespace DeepRockGahactic
{
    ...
    public class DeepRockGahactic
    {
        ...
        public void InitWindow()
        {
            ...
            esp = new Overlay(process.Handle);
        }
        ...
        Int32 EngineLoop()
        {
            ...
            if (!PlayerController.IsA<PlayerController>()) return 1;
            ...
            if (cfg.Ammo)
            {
                ...
                var EquippedItem = InventoryComponent.GetEquippedItem();
            ...
            if (cfg.Console)
            {
                if (!PlayerController.CheatManager.IsA<CheatManager>())
                {
                    ...
                    var console = new SDK.Script.EngineSDK.Console(UnrealEngine.Memory.Execute(UnrealEngine.GStaticCtor,
                        engine.ConsoleClass.Value, engine.GameViewport.Address, 0, 0, 0, 0, 0, 0, 0));
                    engine.GameViewport.ViewportConsole = console;

                    var cheats = new CheatManager(UnrealEngine.Memory.Execute(UnrealEngine.GStaticCtor,
                        PlayerController.CheatClass.Value, engine.GameViewport.Address, 0, 0, 0, 0, 0, 0, 0));
            ...
            if (cfg.Resources)
            {
                ...
                for (var i = 1; i < PlayerResources.Num; i++)
                {
            ...
            if (!PlayerCameraManager.IsA<PlayerCameraManager>()) return 1;
            ...
            var CameraLocation =
                UnrealEngine.Memory.ReadProcessMemory<Vector3>(CameraPOV.Location.Address); // minor optimization
            //var CameraRotation = new Vector3(CameraPOV.Rotation.Yaw, CameraPOV.Rotation.Pitch, CameraPOV.Rotation.Roll);
            var CameraRotation =
                UnrealEngine.Memory.ReadProcessMemory<Vector3>(CameraPOV.Rotation.Address); // minor optimization
            ...
            // if (cfg.Radar) 
            //     esp.DrawArrow(PlayerLocation, CameraRotation, PlayerLocation, CameraRotation);
            ...
            // Owning World
            var actorListOffset =
                (nint)World.PersistentLevel.GetFieldOffset(World.PersistentLevel.GetFieldAddr("OwningWorld")) - 0x10;
            ...
            for (var i = 0; i < Actors.Num; i++)
            {
                ...
                if (Actor.IsA(out DeepPathfinderCharacter character))
                {
                    ...
                    if (RootComponent.Address == 0) continue;
                    if (!RootComponent.IsA<SceneComponent>()) continue;
                    var RelativeLocation = RootComponent.RelativeLocation;
                    if (RelativeLocation.Address == 0) continue;
                ...
                {
                    // var outline = new OutlineComponent(Actor["outline"].Address); 
                    // if (!outline.IsA<OutlineComponent>()) continue;
                ...
                if (cfg.Objectives)
                {
                    if (Actor.IsA(out Gem gem) || Actor.IsA(out TreasureBox box) || Actor.IsA(
                            "BlueprintGeneratedClass /Game/GameElements/Resources/Collectibles/BP_Collectible_Base.BP_Collectible_Base_C"))
                    {
                        if (Actor.bActorIsBeingDestroyed) continue;
                        var RootComponent = Actor.RootComponent;
                        if (RootComponent.Address == 0) continue;
