...
public class SigScan
{
    ...
    public byte[] VDumpedRegion => _vDumpedRegion;
    ...
    private bool MaskCheck(int nOffset, byte[] btPattern, string strMask)
    {
        ...
        for (int x = 0; x < btPattern.Length; x++)
        {
            ...
            if ((strMask[x] == 'x') && (btPattern[x] != _vDumpedRegion[nOffset + x]))
                return false;
        }
    ...
    public IntPtr FindPattern(byte[] btPattern, string strMask, int nOffset)
    {
        try
        {
            ...
            for (int x = 0; x < _vDumpedRegion.Length - strMask.Length; x++)
            {
                if (MaskCheck(x, btPattern, strMask))
                {
                    // The pattern was found, return it.
                    return IntPtr.Add(_vAddress, x + nOffset);
                }
            }
    ...
    public List<IntPtr>? FindPatterns(byte[] btPattern, string strMask, int nOffset)
    {
        ...
        try
        {
            ...
            // Loop the region and look for the pattern.
            for (int x = 0; x < _vDumpedRegion.Length; x++)
            {
                if (MaskCheck(x, btPattern, strMask))
                {
                    // The pattern was found, return it.
                    ptrs.Add(IntPtr.Add(_vAddress, x + nOffset));
                }
            }
